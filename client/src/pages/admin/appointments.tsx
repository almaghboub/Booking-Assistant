...
import { Search, Calendar, Phone, MoreHorizontal, CheckCircle, XCircle, UserX, CreditCard } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const statusConfig: Record<string, { label: string; className: string }> = {
  pending_payment:    { label: "انتظار الدفع",  className: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300" },
...
export default function AdminAppointments() {
  ...
  const markPaidMutation = useMutation({
    mutationFn: (id: string) =>
      apiRequest("PATCH", `/api/admin/appointments/${id}/status`, { status: "confirmed", paymentStatus: "paid" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/appointments"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/appointments/today"] });
      queryClient.invalidateQueries({ queryKey: ["/api/admin/stats"] });
      toast({ title: "تم تأكيد الدفع وتحديث الموعد" });
    },
  });
              ...
              {filtered.map((appt: any) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-4 p-3 rounded-md border bg-card ${isPendingPayment ? "border-orange-200 dark:border-orange-900/40" : "border-border"}`}
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${sc?.className ?? "bg-muted text-muted-foreground"}`}>
                          {sc?.label ?? appt.status}
                        </span>
                        {isPendingPayment && appt.paymentAmount && (
                          <span className="text-xs text-orange-600 dark:text-orange-400 flex items-center gap-1 font-medium" data-testid={`text-payment-amount-${appt.id}`}>
                            <CreditCard className="w-3 h-3" />
                            {parseFloat(appt.paymentAmount).toFixed(2)} ل.د
                          </span>
                        )}
                      </div>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {isPendingPayment && (
                          <>
                            <DropdownMenuItem
                              onClick={() => markPaidMutation.mutate(appt.id)}
                              data-testid={`button-mark-paid-${appt.id}`}
                              className="text-orange-600 dark:text-orange-400 font-medium"
                            >
                              <CreditCard className="w-4 h-4 ml-2" />تأكيد استلام الدفع
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                          </>
                        )}
